import type { Arg } from "../types/ids";
import { Stringf } from "./string_util";
...
export class CustomID<VarInfoModel extends string = string> {
            ...
            for (let i = 0; i < args.length; i++) {
                ...
                if (typeof arg === "string")
                    allArgs = allArgs.concat(
                        CustomID.#ReadID<VarInfoModel>(arg)
                    );
                else allArgs.push(arg);
            }
        }
    ...
    TryFetchVarInfo<values extends string = string>(
        ...
        const variableInfos = this.#args.filter(
            (arg) => typeof arg !== "string"
        ) as VariableInfo<values, VarInfoModel>[];
    ...
    add(...args: Arg<VarInfoModel>[]): this {
        if (args && args.length > 0) {
            for (let i = 0; i < args.length; i++) {
                const arg = args[i];
                if (typeof arg === "string")
                    this.#args = this.#args.concat(CustomID.#ReadID(arg));
    ...
    static #ReadID<VarInfoModel extends string = string>(
        id: string
    ): [...args: Arg<VarInfoModel>[]] {
...
export class VariableInfo<
    ...
    VarInfoModel extends string = string
> {
    [k: number]: values;
    ...
    #setFullVar() {
        let fullVar = "";
        if (
            !Stringf.isNullOrEmpty(this.#key) &&
            !Stringf.isNullOrEmpty(this.#values[0])
        )
            fullVar = `${this.#key}:${this.#values[0]}`;
        if (this.#values.length > 1)
            fullVar = `${this.key}:${this.#values.join(",")}`;
    ...
    #setKeyAndValues() {
        ...
        if (
            !Stringf.isNullOrEmpty(newVarInfo.key) &&
            newVarInfo.values.length > 0
        ) {
...
function SetClassString<T extends CustomID | VariableInfo>(
    _Class: CustomID | VariableInfo,
    value: string
): T {
...
function ReadVariable<values extends string = string>(
    variable: string
): VariableInfo<values> {
    ...
    if (varDeclaration.length === 2) {
        ...
        if (stringValues.length > 0)
            return new VariableInfo<values>(key, ...(stringValues as values[]));
